...
editor opens. The link is relative (the experiment directory gets moved around) and the
helper must never be what breaks the antab step.
"""
...
class TestLinkVixfile:
    ...
    def test_the_link_is_relative(self, exp, tmp_path):
        ...
        assert target == os.path.join('..', 'ES124.vix')
