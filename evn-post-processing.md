...
- *{exp}-auto-{n}.ps* (typically *n=1,2*, for two different scans): Autocorrelations (amplitude vs frequency) for each antenna. Check that all antennas show good bandpasses (ideally flat across the band). Note that observations at L band may show RFI.
- *{exp}-cross-{n}.ps*: Cross-correlations (amplitude and phase versus frequency) for each baseline to the reference antenna. Check that fringes are found (you see clear amplitudes and phases) and the polarizations are OK for all antennas (e.g. no swapped polarizations, and similar signal in both of them).
