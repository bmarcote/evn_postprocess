...
version 4.1 changes
- Adapted for the new archive machine.
version 4.0 changes
- Spectral lines passes now need to be specified via a aurgment.
version 3.0 changes
...
parser.add_argument('-l', '--line', default=False, action='store_true',
                    help='Defines this pass as spectral line one.')
...
def get_input_file_info():
    """Parse the observed sources from the pipeline input file (/data/pipe/{exp}/in/{exp}.inp.txt).
    ...
    """
    with open('/data/pipe/{}/in/{}.inp.txt'.format(args.experiment.lower().split('_')[0],
...
def get_setup():
    ...
    It takes the file {exp}.SCAN that should be in /data/pipe/{exp}/out/.
...
def get_antennas():
    ...
    from the {exp}.DTSUM located in /data/pipe/{exp}/out/.
    """
    with open('/data/pipe/{}/out/{}.DTSUM'.format(args.experiment.lower().split('_')[0],
...
with open(template_comment_file, 'r') as template:
    type_experiment = 'line' if args.line else 'cont'
    ...
    if args.output_comment is None:
        outputdir = '/data/pipe/{}/out'.format(args.experiment.lower().split('_')[0])
    else:
...
with open(template_tasav_file.replace('template', 'template-pbcor' if do_pb_cor else 'template'), 'r') as template:
    ...
    if args.output_tasav is None:
        outputdir = '/data/pipe/{}/in'.format(args.experiment.lower().split('_')[0])
    else:
