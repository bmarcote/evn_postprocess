...
def shell_command(command: str, parameters: Optional[Union[str, list]] = None, shell: bool = True,
    ...
    Args:
        ...
        ok_returncodes (tuple[int, ...]): Exit codes to treat as success besides 0. For the
            few tools that signal something other than failure with a non-zero code (e.g.
            antab_editor.py exits 127 when its window is closed). Each one is logged as a
            warning so it is never silently swallowed.
    ...
    if process.returncode in ok_returncodes:
        logger.warning(f"'{command}' exited with code {process.returncode}, which is expected "
                       f"for this command and not treated as a failure.")
