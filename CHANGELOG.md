...
Changed:
  - The `checklis` step now says what is actually wrong with the .lis files, instead of the
    ...
    (the passes run in parallel, so the output is captured and printed in pass order rather
    than interleaved live); and then a boxed message reviewing them. That box classifies what
    was reported into skipped scans, duplicated data and any other error, and summarises each
