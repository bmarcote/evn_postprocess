...
edit_params = ('refant', 'calsour', 'onebit', 'polswap', 'polconvert',
               'target', 'calibrator', 'fringefinder')
...
all_commands = {'dirs': Command(env.create_all_dirs, "Creates folders in eee and jop83: " \
                ...
                'j2ms2': Command(eee.j2ms2, "Runs j2ms2 with the specified params " \
                                            "(modify them with the 'edit' command)."),
                'expname': Command(eee.update_ms_expname,
                                   "Runs expname.py (for e-EVN experiments)."),
                ...
                'gv': Command(eee.open_standardplot_files,
                              "Opens the standardplots files with gv."),
                ...
                'polconvert': Command(eee.polconvert,
                                      "Runs PolConvert (or prepares files to run it manually)."),
                'postpolconvert': Command(eee.post_polconvert,
                                          "Runs all required steps after run PolConvert."),
                'archive-fits': Command(eee.archive,
                                        "Runs archive in eee on the standard plots and " \
                                        "FITS-IDI files"),
                ...
                'pyinput': Command(pipe.create_input_file,
                                   "Creates the input file for the EVN pipeline."),
                'pipe': Command(pipe.run_pipeline, "Runs the EVN Pipeline."),
                'comment_tasav': Command(pipe.comment_tasav_files,
                                         "Creates the .comment and .tasav file."),
                ...
                'append': Command(eee.append_antab,
                                  "Appends the Tsys and GC to the FITS-IDI files."),
                'ampcal': Command(pipe.ampcal, "Runs ampcal.sh script to incorporate the gain " \
                                               "corrections into Grafana"),
                'pipelet': Command(eee.create_pipelet,
                                   "Creates the piletter_auth containing the credentials."),
                'issues': Command(eee.antenna_feedback,
                                  "Tells you where to store the observed problems " \
                                  "(station feedback, aka to Grafana, and JIVE RedMine."),
                'nme': Command(eee.nme_report, "Tells you if you need to write an NME Report.")}
supsciers = ('agudo', 'bayandina', 'blanchard', 'burns', 'immer', 'marcote',
             'minnie', 'murthy', 'nair', 'oh', 'orosz', 'paragi', 'rmc', 'surcis', 'yang')

breakline = '\n'
help_exec = f"""[bold]Runs a single command of the experiment post-process.[/bold]

This method allows you even more granularity than 'run' as it will only run a single command
...
def run(exp: experiment.Experiment, step1: Optional[str] = None, step2: Optional[str] = None,
    ...
    try:
        ...
        # step1 is not None
            ...
            assert step1 in step_keys, f"The introduced step1 {step1} is not recognized from " \
                                       f"the list {step_keys}."
    ...
    try:
        for a_step in the_steps:
            if not all_steps[a_step](exp):
                raise RuntimeError(f"An error was found in {exp.expname} at the step " \
                                   f"{all_steps[a_step].__name__}")
...
def main():
    ...
    parser.add_argument('-jss', '--supsci', type=str, default=None,
                        ...
                        '[dim]By default recovered assuming you run this ' \
    ...
    parser.add_argument('--no-gui', default=False, action='store_true',
                        help="Choose this option if you don't want graphical windows to be open "\
                             "(e.g. the standanrd plots).")
    parser_run = subparsers.add_parser('run', help='Runs the post-process from a given step.',
    ...
    parser_run.add_argument('step2', type=str, default=None, nargs='?',
                            help='Last step to run (included). ' \
                            'By default None so it will run until the end.')
    parser_exec = subparsers.add_parser('exec',
                                        help='Executes a single function from post-processing.',
                                        description=help_exec,
                                        formatter_class=parser.formatter_class)
    _ = subparsers.add_parser('info',
                                        help='Shows the metadata associated to the experiment',
                                        description=help_info,
                                        formatter_class=parser.formatter_class)
    _ = subparsers.add_parser('last', help='Returns the last step that run ' \
                                        'successfully from the post-process', description=help_last,
                                        formatter_class=parser.formatter_class)
    parser_edit = subparsers.add_parser('edit',  help='Edits one of the parameters ' \
    ...
    parser_exec.add_argument('command', type=str,
                             help='Runs a single command from the experiment post-processing.')
    ...
    try:
        assert env.grep_remote_file('jops@ccs', '/ccs/var/log2vex/MASTER_PROJECTS.LIS',
                                    args.expname.upper()) != '', \
            f"The experiment name {args.expname} is not recognized " \
    ...
    assert args.supsci in supsciers, "It seems like the JIVE Support Scientist " \
                                     f"{args.supsci} is not recognized " \
                                     "from my database. You may need to use the " \
                                     "--supsci option or ask for support."
    ...
    if args.subpar is None:
        ...
        assert args.command in all_commands.keys(), \
               f"The provided command {args.command} is not recognized." \
               f"Accepted commands are: {', '.join(all_commands.keys())}. " \
               "Run 'postprocess exec -h' for more info."
